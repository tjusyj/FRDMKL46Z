...
struct mediana_type arr_medianas2[3];
unsigned int cat;
unsigned int offset;
...
void angle_calculation(void)
 ...
 /*  if (accel_count != 0) return;
   ...
   */
   ...
   nv = (signed char)(Aresultx);
   X_acc = median(nv, &arr_medianas2[0]);
   
   
   nv = (signed char)(Aresulty);
   Y_acc = median(nv, &arr_medianas2[1]);
   
   nv = (signed char)(Aresultz);  
   Z_acc = median(nv, &arr_medianas2[2]);
