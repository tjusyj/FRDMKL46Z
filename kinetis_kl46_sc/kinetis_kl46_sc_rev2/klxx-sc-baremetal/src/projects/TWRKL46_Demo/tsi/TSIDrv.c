...
void TSI_SelfCalibration(void)
{
  ...
  if(trigger_backup)                      // Restore trigger mode
   ...
   // TSI0_GENCS |= TSI_GENCS_TSIEN_MASK;     // Enable TSI module 
   // TSI0_DATA |= TSI_DATA_SWTS_MASK;      //LPP software scan initialized in application 
  
}


void vfn_start_tsi_scan(void)
{
  TSI0_GENCS |= TSI_GENCS_TSIEN_MASK;     // Enable TSI module 
  TSI0_DATA |= TSI_DATA_SWTS_MASK;      //LPP software scan initialized in application 
}
...
///            TSI0_DATA |= TSI_DATA_SWTS_MASK;   //LPP initialized in application for multiplexing with sLCD
            TSI0_GENCS &= ~TSI_GENCS_TSIEN_MASK;     // Enable TSI module
    }
}
...
void TSI_Electrode_detection(void)
{
       if(end_flag)
      {
         end_flag = FALSE;
         if(gu16Delta[0] > gu16Threshold[0])  Electrode1 = 1; else Electrode1=0;
        
         if(gu16Delta[1] > gu16Threshold[1])
                 Electrode2 = 1; else Electrode2=0;
      }
